...
impl Snapshot {
    ...
    pub fn from_store(store: &NodeStore, description: Option<String>) -> Result<Self> {
        ...
        let id = format!("snap_{}", uuid::Uuid::new_v4().to_string().replace("-", "")[..12].to_string());
    ...
    pub fn incremental_from_store(
        ...
        snapshot.metadata.id = format!("snap_inc_{}", uuid::Uuid::new_v4().to_string().replace("-", "")[..8].to_string());
        Ok(snapshot)
    }
    ...
    pub fn save_to_file<P: AsRef<Path>>(&mut self, path: P) -> Result<()> {
        ...
        let json_for_checksum = serde_json::to_vec_pretty(self).map_err(|e| {
            ReasonError::Backup(format!("Failed to serialize snapshot: {}", e))
        })?;
        ...
        let file = File::create(path).map_err(|e| {
            ReasonError::Backup(format!("Failed to create backup file: {}", e))
        })?;
        ...
        encoder.write_all(&json).map_err(|e| {
            ReasonError::Backup(format!("Failed to write backup: {}", e))
        })?;
        encoder.finish().map_err(|e| {
            ReasonError::Backup(format!("Failed to finalize backup: {}", e))
        })?;
    ...
    pub fn load_from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let file = File::open(path).map_err(|e| {
            ReasonError::Backup(format!("Failed to open backup file: {}", e))
        })?;
        ...
        decoder.read_to_end(&mut json).map_err(|e| {
            ReasonError::Backup(format!("Failed to decompress backup: {}", e))
        })?;
        
        // Deserialize
        let snapshot: Snapshot = serde_json::from_slice(&json).map_err(|e| {
            ReasonError::Backup(format!("Failed to deserialize snapshot: {}", e))
        })?;
        ...
        let verify_json = serde_json::to_vec_pretty(&verify_snapshot).map_err(|e| {
            ReasonError::Backup(format!("Failed to verify snapshot: {}", e))
        })?;
